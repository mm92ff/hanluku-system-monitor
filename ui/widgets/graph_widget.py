...
class GraphWidget(QWidget):
    ...
    def set_data(self, data: Dict[str, List[Tuple[float, float]]], current_unit: str = ""):
    ...
    def _get_data_bounds(self) -> Tuple[float, float, float, float]:
        """Ermittelt die Min/Max-Werte fuer Werte und Zeitstempel."""
        all_values = [point[1] for points in self.data.values() for point in points]
        all_timestamps = [point[0] for points in self.data.values() for point in points]
    ...
    def _draw_axes_and_grid(
        ...
        for i in range(num_y_labels + 1):
            ...
            y = self.height() - self.padding - (val - min_v) / (max_v - min_v) * (
                self.height() - 2 * self.padding
            )
            painter.drawLine(self.padding, y, self.width() - self.padding, y)

            label = f"{val:.0f}{self.current_unit}"
            painter.drawText(
                self.padding - fm.horizontalAdvance(label) - 5,
                y + fm.height() / 4,
                label,
            )
        ...
        for i in range(num_x_labels + 1):
            ...
            x = self.padding + (ts - min_t) / (max_t - min_t) * (
                self.width() - 2 * self.padding
            )
            if i > 0:
                painter.drawLine(x, self.padding, x, self.height() - self.padding)
            label = time.strftime("%H:%M", time.localtime(ts))
            painter.drawText(
                x - fm.horizontalAdvance(label) / 2,
                self.height() - self.padding + fm.height() + 5,
                label,
            )
    ...
    def _draw_no_data_text(self, painter: QPainter):
        ...
        text = (
            self.translator.translate("graph_no_data")
            if self.translator
            else "Select a sensor to display its history."
        )
